...
EXPECTED_CANONICAL_SKILLS = frozenset(
    {
        "github-delivery",
...
# GitHub Actions references are checked only for step-level ``uses`` keys.
# Reusable workflow jobs, local actions, and Docker containers have different
# reference semantics and are intentionally classified separately.
...
def _validate_task_state_shape(
    ...
    if isinstance(shape, dict):
        ...
        if missing or unknown:
            fail(
                f"{rel(path, root)} task-state {label} keys invalid: "
                f"missing={missing} unknown={unknown}"
            )
...
def _validate_task_state_document(value: Any, path: Path, root: Path) -> None:
    if not isinstance(value, dict) or any(not isinstance(key, str) for key in value):
    ...
    if missing or unknown:
        fail(
            f"{rel(path, root)} task-state document keys invalid: "
            f"missing={missing} unknown={unknown}"
        )
...
def validate_task_state(path: Path, root: Path = ROOT) -> None:
    ...
    if state["completed_evidence"]:
        ...
        if not closure["paths"] or not closure["conditions"]:
            fail(
                f"{rel(path, root)} completed evidence requires paths and "
                "conditions in input closure"
            )
    ...
    if status == "complete" and (
        ...
        fail(
            f"{rel(path, root)} complete task-state must have no remaining work, "
            "invalidated gates, or blockers"
        )
    if status == "complete" and (
...
def validate_frontmatter(path: Path, root: Path) -> dict[str, Any]:
    ...
    if len(parts) >= 3 and parts[:2] in ((".agents", "skills"), (".claude", "skills")):
        ...
        if not (
            (isinstance(globs, str) and globs.strip())
            or (
...
def validate_skills(
    ...
    *,
    ...
    canonical_identities = _collect_skill_identities(
        canonical, root, "canonical Skill", "skill"
    )
    adapter_identities = _collect_skill_identities(
        adapters, root, "Claude adapter", "adapter"
    )
    if expected_names is not None and set(canonical_identities) != set(expected_names):
        missing = sorted(set(expected_names) - set(canonical_identities))
        unexpected = sorted(set(canonical_identities) - set(expected_names))
        fail(f"canonical Skill set invalid: missing={missing} unexpected={unexpected}")
...
def _iter_step_uses_lines(yml: str) -> list[tuple[object, str]]:
    """Return step-level YAML ``uses`` keys and their source lines."""

    try:
        root = yaml.compose(yml, Loader=yaml.SafeLoader)
    except yaml.YAMLError as error:
        fail(f"workflow is not valid YAML: {error}")
    if root is None:
        return []
    ...
    def visit(node: yaml.Node, context: str = "root") -> None:
        ...
        if isinstance(node, yaml.MappingNode):
            for key, value in node.value:
                ...
                if context == "step" and key_name == "uses":
                    source_line = lines[key.start_mark.line]
                    references.append(
                        (
                            value.value if isinstance(value, yaml.ScalarNode) else None,
                            source_line,
                        )
                    )
                child_context = "other"
...
def external_action_pin_violations(yml: str) -> list[str]:
    """Return violations for external step-level GitHub Action references."""
    ...
    for value, source_line in _iter_step_uses_lines(yml):
        ...
        if reference_kind == "unknown":
            ...
            violations.append(f"{display_value}: unsupported step-level uses reference")
            continue
...
# Keep the source-level name used by the former focused workflow contract
# tests available while the implementation lives in this central validator.
_external_action_pin_violations = external_action_pin_violations
...
def validate_workflow_action_pins(
    paths: Path | Iterable[Path], root: Path = ROOT
) -> None:
    """Validate external step-level ``uses`` references in workflow files."""

    candidates = [paths] if isinstance(paths, Path) else list(paths)
    for path in candidates:
        require_file(path, root)
        for violation in external_action_pin_violations(text(path)):
            fail(f"{rel(path, root)}: {violation}")


def validate_workflow_actions(root: Path = ROOT) -> int:
    """Validate all repository workflow files and return their count."""

    workflows = discover_workflows(root)
    validate_workflow_action_pins(workflows, root)
    return len(workflows)


def validate_action_pins(root: Path = ROOT) -> int:
    """Compatibility wrapper for callers that refer to the action-pin gate."""

    return validate_workflow_actions(root)
...
def validate_repository(root: Path) -> tuple[int, int, int]:
    ...
    counts = validate_skills(root, routers, expected_names=EXPECTED_CANONICAL_SKILLS)
    canonical = sorted((root / ".agents/skills").glob("*/SKILL.md"))
    adapters = sorted((root / ".claude/skills").glob("*/SKILL.md"))
    validate_links([*files, *rules, *routers, *canonical, *adapters], root)
    validate_workflow_actions(root)
    return counts
...
def main() -> int:
    ...
    try:
        ...
        return 1
    print(
        f"Governance verification: PASS "
        f"({counts[0]} Skills, {counts[1]} adapters, {counts[2]} routers)"
    )
    return 0
