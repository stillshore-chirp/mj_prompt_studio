...
import pytest
from scripts.validate_governance import TASK_STATE_TEMPLATE, GovernanceError, validate_task_state
...
def test_task_state_template_is_valid_synthetic_and_client_neutral(tmp_path: Path) -> None:
    ...
    assert (
        state["completed_evidence"][0]["artifact_reference"]
        in state["input_closure"]["artifacts"]
    )
