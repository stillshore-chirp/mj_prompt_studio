...
APIキーがなくても、Mock LLMモードで画面の流れを試せます。アプリを起動した後は、右下の「使い方」からこの最初に試す流れとユーザーマニュアルをいつでも開けます。現在の画面に合った説明へ移動する場合は、ヘルプpanelの「この画面の使い方へ」を使います。
...
4. 環境変数を使わない場合は、`API Key`欄に入力して`このセッションだけで使用`を押す。保存する場合は`OS資格情報ストアへ保存`を押す。保存済みのキーを使う場合は`OS資格情報ストアから読み込んで使用`を押す。
...
- プロンプトが長い: 各Blockの`短縮`、または`Prompt Workshop`の`文字数のみ調整`を使う。
