...
- LLM Job失敗時は下部Jobsとstatusに、対象操作、結果が適用されていないこと、安全な原因カテゴリ、次の回復操作を表示する。API key未設定・初期化・認証・権限・利用上限・ネットワーク・実API一時障害・リクエスト・応答保存・構造化schema・構造化応答・未分類を区別し、生のprovider例外、入力本文、識別子は表示しない。
- 設定確認が必要な失敗だけに「設定を開く」を出し、待機が適切な利用上限・一時障害には自動再試行をせず待機を案内する。再試行は入力を保持した明示操作だけで行う。
