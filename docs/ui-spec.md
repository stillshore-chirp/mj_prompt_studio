...
- LLM Job失敗時は下部Jobsとstatusに、対象操作、結果が適用されていないこと、安全な原因カテゴリ、次の回復操作を表示する。API key未設定・初期化・認証・権限・利用枠または請求上限・一時的なリクエスト制限・ネットワーク・実API一時障害・リクエスト・応答保存・構造化schema・構造化応答・未分類を区別し、生のprovider例外、入力本文、識別子は表示しない。
- 設定確認が必要な失敗だけに「設定を開く」を出す。利用枠または請求上限ではOpenAI Platformの利用枠・請求状態を確認してから再試行し、一時的なリクエスト制限・一時障害では自動再試行をせず待機を案内する。再試行は入力を保持した明示操作だけで行う。
- 失敗詳細では原因、次にできること、再試行判断に加え、安全な失敗段階、HTTP状態、allowlist済みproviderコードを表示する。「診断情報をコピー」は入力本文、API key、生のprovider応答、request/job/trace IDを含めず、Clipboard API失敗時は読み取り専用textareaで手動コピーできる。
- 診断情報のない旧履歴では原因を復元できないことを明記し、その履歴の再試行や根拠のないSettings確認を勧めない。新しく1回だけ実行して得た診断情報で判断する。
- 構造化応答の再試行は同じJobにつき1回までとし、再失敗後は再試行操作を出さず診断情報の確認・報告へ切り替える。再試行リクエスト中はボタンを無効化し、serverもfailed状態と上限をロック内で検証して二重送信を拒否する。失敗分類に固有の復旧案内をallowlist済みproviderコードの補足より優先し、応答保存拒否ではPrivacy mode導線を保持する。
...
- 実行状態: 設定モード、実行バックエンド（実API / 明示的Mock / 利用不可）、キー設定有無、資格情報ストアの安全な状態、接続テストの安全な結果を表示する。Mockの接続テストは成功として表示しない。接続テストはAPI keyと基本接続だけを確認し、構造化AI Jobや現在の応答保存設定の成功まで保証しないことを明記する。
