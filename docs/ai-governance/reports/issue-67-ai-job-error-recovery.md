...
- 対象Issue / PR / 作業: #67 / #68 / LLM failure boundary、JobsPanel、Settings、App status
...
- 熟練者効率: 再試行は入力を保持し、失敗cardから明示的に開始する。一時的なリクエスト制限・一時障害には再試行を出さず、操作の繰り返しを抑える。利用枠・請求上限は待機だけで回復しないため、確認後に同じJobを再試行できる。
